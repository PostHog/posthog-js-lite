...
export const decideImplementation = (flags: any, decideStatus: number = 200) => (url: any) => {
...
describe('local evaluation', () => {
    ...
    it('evaluates person properties', async () => {
        ...
        expect(await posthog.getFeatureFlag("person-flag", "some-distinct-id", false, undefined, {"region": "USA"})).toEqual(true)
        expect(await posthog.getFeatureFlag("person-flag", "some-distinct-id", false, undefined, {"region": "Canada"})).toEqual(false)
    ...
    it('evaluates group properties', async () => {
        ...
        expect(await posthog.getFeatureFlag("group-flag", "some-distinct-id", false, {}, {}, {"company": {"name": "Project Name 1"}})).toEqual(false)
        expect(await posthog.getFeatureFlag("group-flag", "some-distinct-2", false, {}, {}, {"company": {"name": "Project Name 2"}})).toEqual(false)

        // # this is good
        expect(await posthog.getFeatureFlag("group-flag", "some-distinct-2", false, {"company": "amazon_without_rollout"}, {}, {"company" : {"name" : "Project Name 1"}})).toEqual(true)
        
        // # rollout % not met
        expect(await posthog.getFeatureFlag("group-flag", "some-distinct-2", false, {"company" : "amazon"}, {}, {"company" : {"name" : "Project Name 1"}})).toEqual(false)
        
        // # property mismatch
        expect(await posthog.getFeatureFlag("group-flag", "some-distinct-2", false, {"company" : "amazon_without_rollout"}, {}, {"company" : {"name" : "Project Name 2"}})).toEqual(false)
    ...
    it('evaluates group properties and falls back to decide when group_type_mappings not present', async () => {
        ...
        expect(await posthog.getFeatureFlag("group-flag", "some-distinct-2", false, {}, {}, {"company" : {"name" : "Project Name 1"}})).toEqual('decide-fallback-value')
    })
    ...
    it('evaluates flag with complex definition', async () => {
        ...
        expect(await posthog.getFeatureFlag("complex-flag", "some-distinct-id", false, {}, {"region": "USA", "name": "Aloha"})).toEqual(true)
        expect(mockedUndici.fetch).not.toHaveBeenCalled()
        
        // # this distinctIDs hash is < rollout %
        expect(await posthog.getFeatureFlag("complex-flag", "some-distinct-id_within_rollout?", false, {}, {"region": "USA", "email": "a@b.com"})).toEqual(true)
        expect(mockedUndici.fetch).not.toHaveBeenCalled()
        
        // # will fall back on `/decide`, as all properties present for second group, but that group resolves to false
        expect(await posthog.getFeatureFlag("complex-flag", "some-distinct-id_outside_rollout?", false, {}, {"region": "USA", "email": "a@b.com"})).toEqual("decide-fallback-value")
        console.log(mockedUndici.fetch.mock.calls)
        expect(mockedUndici.fetch).toHaveBeenCalledWith("http://example.com/decide/?v=2", expect.objectContaining({ "body": JSON.stringify({
            "token": "TEST_API_KEY", "distinct_id": "some-distinct-id_outside_rollout?", "groups": {}, "person_properties": {"region": "USA", "email": "a@b.com"}, "group_properties": {},})}
        ))
        mockedUndici.fetch.mockClear()
        
        // # same as above
        expect(await posthog.getFeatureFlag("complex-flag", "some-distinct-id", false, {}, {"doesnt_matter": "1"})).toEqual("decide-fallback-value")
        expect(mockedUndici.fetch).toHaveBeenCalledWith("http://example.com/decide/?v=2", expect.objectContaining({ "body": JSON.stringify({
            "token": "TEST_API_KEY", "distinct_id": "some-distinct-id", "groups": {}, "person_properties": {"doesnt_matter": "1"}, "group_properties": {}}
        )}))
        mockedUndici.fetch.mockClear()

        
        expect(await posthog.getFeatureFlag("complex-flag", "some-distinct-id", false, {}, {"region": "USA"})).toEqual("decide-fallback-value")
        ...
        expect(await posthog.getFeatureFlag("complex-flag", "some-distinct-id_outside_rollout?", false, {}, {"region": "USA", "email": "a@b.com", "name": "X", "doesnt_matter": "1"})).toEqual(false)
        expect(mockedUndici.fetch).not.toHaveBeenCalled()
    ...
    it('falls back to decide', async () => {
        ...
        mockedUndici.fetch.mockImplementation(decideImplementation({"beta-feature": "alakazam", "beta-feature2": "alakazam2"}))
    ...
    it('dont fall back to decide when local evaluation is set', async () => {
        ...
        // # but doesn't because only_evaluate_locally is true
        expect(await posthog.getFeatureFlag("beta-feature2", "some-distinct-id", false, {}, {}, {}, true)).toEqual(undefined)
    ...
    it('get all flags with fallback', async () => {
        ...
        mockedUndici.fetch.mockImplementation(decideImplementation({"beta-feature": "variant-1", "beta-feature2": "variant-2"}))
        ...
        expect(await posthog.getAllFlags("distinct-id")).toEqual({"beta-feature": "variant-1", "beta-feature2": "variant-2", "disabled-feature": false})
        expect(mockedUndici.fetch).toHaveBeenCalledTimes(1)
        mockedUndici.fetch.mockClear()
    })
    ...
    it('get all flags with fallback but only_locally_evaluated set', async () => {
        ...
        expect(await posthog.getAllFlags("distinct-id", {}, {}, {}, true)).toEqual({"beta-feature": true, "disabled-feature": false})
        expect(mockedUndici.fetch).not.toHaveBeenCalled()
    })
    ...
    it('get all flags with fallback, with no local flags', async () => {
        ...
        expect(await posthog.getAllFlags("distinct-id")).toEqual({"beta-feature": "variant-1", "beta-feature2": "variant-2"})
        expect(mockedUndici.fetch).toHaveBeenCalledTimes(1)
        mockedUndici.fetch.mockClear()
    })
...
describe.skip('match properties', () => {
    ...
    it('with operator regex', () => {
        ...
        expect(matchProperty(property_a, { 'key' : 'value\com' })).toBe(false)
...
describe.skip('consistency tests', () => {
    ...
    it('is consistent for multivariate flags', () => {
        mockedUndici.fetch.mockImplementation((url) => {
            if ((url as any).includes('/decide/')) {
              return Promise.resolve({
                statusCode: 400,
        ...
        results.forEach(async (result, index) => {
            const distinctId = `distinct_id_${index}`
            const value = await posthog.getFeatureFlag('simple-flag', distinctId)
            expect(value).toBe(result)
        })
