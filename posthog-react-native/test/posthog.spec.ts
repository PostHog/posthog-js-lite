...
import { PostHog, PostHogStorage, PostHogOptions } from "../index";
...
describe("PostHog React Native", () => {
  let mockStorage: PostHogStorage;
  ...
  describe("screen", () => {
    it("should set a $screen_name property on screen", async () => {
      ...
      expect(
        posthog.getPersistedProperty(PostHogPersistedProperty.Props)
      ).toEqual(undefined);
    });
  });
  ...
  describe("captureNativeAppLifecycleEvents", () => {
    ...
    it("should track app background and foreground", async () => {
      ...
      const cb: (state: AppStateStatus) => void = (
