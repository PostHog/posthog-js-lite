...
export class PostHog extends PostHogCore {
  ...
  static async initAsync(
    ...
    if (!posthog) {
      ...
      if (persistence === "file") {
        try {
          const storage = new PostHogSemiAsyncStorage(
            options?.customAsyncStorage || buildOptimisiticAsyncStorage()
          );
          posthog = storage
            .preloadAsync()
            .then(() => new PostHog(apiKey, options, storage));
        } catch (error) {
          console.error(
  ...
  constructor(
    ...
    if (storage) {
      ...
      this._persistence =
        options?.persistence === "memory" ? "memory" : "async";
    }
      ...
      if (!this._storage?.getItem(PostHogPersistedProperty.AnonymousId)) {
        ...
        if (legacyValues?.distinctId) {
          this._storage?.setItem(
            ...
            legacyValues.distinctId
          );
          this._storage?.setItem(
            PostHogPersistedProperty.AnonymousId,
            legacyValues.anonymousId
          );
        }
      }
  ...
  captureNativeAppLifecycleEvents() {
    ...
    const prevAppBuild = this.getPersistedProperty(
      PostHogPersistedProperty.InstalledAppBuild
    );
    const prevAppVersion = this.getPersistedProperty(
